...
import type { Subtask, Task, TaskPriority, TaskStatus } from "@/shared/api/tasks"
import { deadlineToIso, formatDeadline, isOverdue, parseTags, priorityClasses, priorityLabels } from "./task-meta"
...
export function TaskDetailsDialog({
	...
	const [deadlineDraft, setDeadlineDraft] = useState<Date | undefined>(undefined)
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-4xl overflow-hidden border-white/10 bg-slate-950/95 text-white shadow-2xl shadow-black/60 backdrop-blur-2xl">
				{task ? (
					<>
						...
						<div className="relative space-y-6">
							<DialogHeader>
								...
								<DialogTitle className="text-3xl text-white md:text-4xl">{task.title}</DialogTitle>
								<DialogDescription className="max-w-2xl text-white/60">
									Edit the task details, move its status, and manage subtasks. Everything stays inside one cinematic workspace.
								</DialogDescription>
							</DialogHeader>
							...
							<div className="grid gap-3 md:grid-cols-3">
								<div className="rounded-2xl border border-white/10 bg-white/5 p-3">
									...
									<div className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-[11px] uppercase tracking-[0.16em] ${priorityClasses[task.priority]}`}>
								...
								<div className="rounded-2xl border border-white/10 bg-white/5 p-3">
									...
									<div className="mt-2 flex flex-wrap gap-1.5">
										{task.tags.length > 0 ? (
											task.tags.map((tag) => (
												<span key={tag} className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-2 py-1 text-[11px] text-cyan-100">
													#{tag}
												</span>
											))
										) : (
								...
								<div className={`rounded-2xl border p-3 ${task.deadline ? (isOverdue(task.deadline) && task.status !== "completed" && task.status !== "closed" ? "border-rose-400/25 bg-rose-400/10" : "border-white/10 bg-white/5") : "border-white/10 bg-white/5"}`}>
									<div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-white/40">
										<CalendarClock size={13} className={task.deadline ? "text-cyan-300" : "text-white/35"} />
										Deadline
									</div>
									<div className={`mt-2 text-sm ${task.deadline ? "text-white/85" : "text-white/35"}`}>
										{formatDeadline(task.deadline) || "No deadline"}
										{task.deadline && isOverdue(task.deadline) && task.status !== "completed" && task.status !== "closed" ? " overdue" : ""}
									</div>
								</div>
							</div>
								...
								<div className="space-y-4">
									...
									<div className="grid gap-3 md:grid-cols-2">
										...
										<div className="space-y-2">
											...
											<DeadlinePicker value={deadlineDraft} onChange={updateDeadline} />
										</div>
									</div>

									<div className="space-y-2">
										<div className="text-sm uppercase tracking-[0.24em] text-white/50">Status</div>
										<div className="grid grid-cols-2 gap-2">
											{Object.entries(kanbanStatusLabels).map(([status, label]) => (
												<Button
													key={status}
													type="button"
													variant={statusDraft === status ? "default" : "outline"}
									...
									<div className="rounded-2xl border border-white/10 bg-white/5 p-4">
										<div className="mb-2 text-sm uppercase tracking-[0.24em] text-white/50">Meta</div>
										<div className="space-y-2 text-sm text-white/70">
											...
											<div>Tags: {task.tags.length ? task.tags.map((tag) => `#${tag}`).join(", ") : "None"}</div>
											<div>
												Deadline: {formatDeadline(task.deadline) || "No deadline"}
												{task.deadline && isOverdue(task.deadline) && task.status !== "completed" && task.status !== "closed"
													? " (overdue)"
													: ""}
											</div>
										</div>
								...
								<div className="space-y-4">
									<div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-4">
										<div className="flex items-center justify-between gap-3">
											<div>
												<div className="text-sm uppercase tracking-[0.24em] text-white/50">Subtasks</div>
												<div className="text-sm text-white/60">
													{subtasks.filter((subtask) => subtask.completed).length}/{subtasks.length} completed
												</div>
											</div>
											<div className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs text-white/70">{subtasks.length} total</div>
										</div>
										...
										<div className="flex gap-2">
											<Input
												...
												className="border-white/20 bg-white/10 text-white placeholder:text-white/40"
											/>
											<Button type="button" onClick={addSubtask} className="bg-gradient-to-r from-purple-500 to-pink-500 hover:opacity-90">
												Add
											</Button>
										</div>
										...
										<div className="space-y-2">
											...
											{subtasks.map((subtask) => (
												<div key={subtask.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-3 py-2">
													<button
														type="button"
													...
													/>
													<div className={`flex-1 text-sm ${subtask.completed ? "text-white/45 line-through" : "text-white"}`}>
														{subtask.title}
													</div>
													<Button type="button" variant="ghost" onClick={() => onDeleteSubtask(subtask.id)} className="text-white/60 hover:bg-white/10 hover:text-red-400">
														<Trash size={16} />
													</Button>
												</div>
											))}
